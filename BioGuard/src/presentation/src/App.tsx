...
interface AgentInsight { agent: string; message: string; confidence: number; timestamp: string; }

type AppView = 'dashboard' | 'history' | 'scenarios' | 'settings';

function App() {
  const [currentView, setCurrentView] = useState<AppView>('dashboard');
  const [bioState, setBioState] = useState({ glucose: 100, source: 'Sarah_M_47 (Ready)', patient_id: 'PT-2026-SARAH' });
  ...
  const triggerRehearsal = async () => {
    ...
    try {
      ...
      if (response.data.brief?.anomaly_signals?.[0]) {
        setBioState(prev => ({ ...prev, source: `Sarah_M_47 | ADE Signal Detected` }));
      }
    } catch (err) { 
